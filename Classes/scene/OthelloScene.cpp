...
void OthelloLayer::update(float delta) {
    if (!Singleton<Othello>::getInstance()->getIsGameRun()) {
        log("Game Did Finish!");
        auto scene = GameFinishLayer::createScene(Singleton<Othello>::getInstance()->getBoard()->getPlayerScoreInfo());
	...
	if (this->_storedBoardState == nullptr || this->_storedBoardState != currentBoardState) {
        ...
        auto scoreInfo = Singleton<Othello>::getInstance()->getBoard()->getPlayerScoreInfo();
        this->_userScoreMap[Player::BlackPlayer]->setString(string_patch::to_string(scoreInfo.getScore(Player::BlackPlayer)));
        this->_userScoreMap[Player::WhitePlayer]->setString(string_patch::to_string(scoreInfo.getScore(Player::WhitePlayer)));
